...
not the multi-GB OUTPUT tree the pipeline generates (HCR FISH masks alone are ~600 MB).
...
The per-cell 2P→HCR FISH matching table produced at
`demo_pre_run/JS078_demo/OUTPUT/MERGED/aligned_masks/twop_plane0_to_HCR01.csv`.
Each row is a candidate 2P↔HCR FISH mask pair; `is_best_match == True` marks the accepted
1:1 match, and the `somaprint_*` columns are the parallel geometric matcher's call.
(The full per-cell feature table joined to the 5 HCR FISH gene channels is also produced,
...
| quantity | value |
|---|---|
| HCR FISH cells segmented (whole 3D volume, all 39 z-slices) | ~21,000 |
...
| 2P→HCR FISH cascade final overlay IoU | ~0.48 |
| global coarse shift found | dx ≈ −18 px, accepted (no `[WARN] Global rejected`) |

Note the two cell counts are not comparable directly: the ~21,000 HCR FISH cells span the whole
...
value, the 2P→HCR FISH overlay is correct. A railed shift (e.g. `dy=+100`) that gets
