...
two-photon image and the HCR FISH volume. The pipeline pauses and asks for them the first
...
- **Spread.** Cover the whole field, including near the edges. Points clustered in the
  ...
  Do not crop the HCR FISH volume to save space: the registration uses the surrounding tissue
...
- **What to match.** Cell bodies and vasculature that you can identify unambiguously in
  ...
  bright object.
- **Z.** The 2P image is a single plane, so its `z` is 0 throughout. Set the HCR FISH `z` to the
...
| 6, 7, 8 | HCR FISH x, y, z | **microns** |

Rows with `enabled` set to `false` are ignored, so you can disable a bad point without
deleting it.

### The HCR FISH volume must be calibrated

The units in columns 6 to 8 are whichever units the volume declares. If the HCR FISH TIFF
...
- **Overlay looks right, merged table looks wrong.** Suspect z. Confirm the HCR FISH `z` values
